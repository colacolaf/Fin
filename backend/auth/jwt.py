...
def create_access_token(user_id: str) -> str:
    ...
    return jwt.encode(
        {"sub": user_id, "exp": expire, "type": "access", "jti": _jti(), "aud": _AUD, "iss": _ISS},
        settings.jwt_secret,
        algorithm=ALGORITHM,
    )
...
def create_refresh_token(user_id: str) -> str:
    ...
    return jwt.encode(
        {"sub": user_id, "exp": expire, "type": "refresh", "jti": _jti(), "aud": _AUD, "iss": _ISS},
        settings.jwt_secret,
        algorithm=ALGORITHM,
    )
...
def decode_token(token: str) -> dict:
    """Return payload or raise JWTError. Validates audience + issuer."""
    return jwt.decode(
        token,
        settings.jwt_secret,
